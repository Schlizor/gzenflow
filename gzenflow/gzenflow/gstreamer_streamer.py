...
#                 "videoconvert", "!",
#                 "x264enc",
#                 f"bitrate={max_br}", "tune=zerolatency", "speed-preset=ultrafast", "!",
#                 "rtph264pay", "config-interval=1", "pt=96", "!",
#                 f"udpsink", f"host={target_ip}", f"port={port}"
#             ]
#
#             self.logger.info(f"📡 Starte GStreamer Stream {name} auf Port {port}")
#             proc = subprocess.Popen(cmd)
#             self.active_processes[name] = proc
#
#     def _stop_all(self):
#         for name, proc in self.active_processes.items():
#             self.logger.info(f"🛑 Stoppe GStreamer Stream {name}")
#             proc.terminate()
